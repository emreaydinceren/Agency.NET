...
public sealed class SqliteKVStoreFunctionalTests : IClassFixture<SqliteKVStoreFunctionalTests.KVStoreFixture>
{
    ...
    public async Task SearchAsync_WithValueSubstring_Matches()
    {
        ...
        await this._fixture.KVStore.UpsertAsync(keyWithRelease, new { note = "important release notes" }, cancellationToken: TestContext.Current.CancellationToken);
        await this._fixture.KVStore.UpsertAsync(keyUnrelated, new { note = "unrelated" }, cancellationToken: TestContext.Current.CancellationToken);

        var results = await this._fixture.KVStore.SearchAsync<dynamic>(
            new Query(null, "release", null, 100),
            TestContext.Current.CancellationToken);
    ...
    public async Task SearchAsync_OrderedByUpdatedOnDesc()
    {
        ...
        await this._fixture.KVStore.UpsertAsync(keyOld, new { label = "older" }, cancellationToken: TestContext.Current.CancellationToken);

        // Force a 1-second gap so SQLite's datetime('now') produces a different value
        await Task.Delay(TimeSpan.FromSeconds(1), TestContext.Current.CancellationToken);

        await this._fixture.KVStore.UpsertAsync(keyNew, new { label = "newer" }, cancellationToken: TestContext.Current.CancellationToken);

        var results = await this._fixture.KVStore.SearchAsync<dynamic>(
            new Query(null, null, null, 100),
            TestContext.Current.CancellationToken);
