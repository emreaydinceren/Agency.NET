...
| `/dump-context` | | Print the exact request last submitted to the model (not added to history) |
...
`DumpContextCommand.Run` reads `ChatSession.LastLlmRequest` — the request the agent loop recorded immediately before submitting it — deserializes it via `LlmRequestSnapshotCodec`, and hands it to `DumpContextCommand.Render`. Before the first turn there is nothing to report and the command says so. The render prints a header (iteration, model, client type, token cap, capture time) followed by the recorded system prompt, the messages as they stood at submission (text, tool-calls, tool-results), and the advertised tools. Tools are grouped by origin — *Built-in* first, then each MCP server in configured order — by reconstructing provenance from `McpClientPool.ToolNamesByServer` (the flat registry and `ToolDefinition` carry none). Single-line descriptions render inline as `name: summary`; the progressive-discovery `{"type":"object"}` placeholder schema is suppressed. Display-only — it changes nothing sent to the model.

`Render` takes only an `IChatOutput` plus plain data (the snapshot and the tool→server map), so it is unit-testable without a live session; `Run` keeps the DI work. See `DumpContextRenderTests`.
...
- **`/dump-context` replays a recording, it does not rebuild a projection** — it used to call `PreviewContext()` and re-run `SystemPromptBuilder.Build`, which rendered the blackboard *as it stood between turns*. That can never match what the model saw: retrieval runs in `OnPreIteration`, *inside* the iteration, so the `## Facts` / `## Memories` it injects appear only after the user has already typed the command. The loop now serializes the assembled request at the call site and the command prints that. The cost is one capture per iteration; the benefit is that the command is honest by construction rather than by keeping a projection in sync with the loop.
...
- **`/mcp-list` reads the same `ToolNamesByServer` source, plus `DisabledServers`** — server on/off state is *derived*, never stored: `IToolRegistry.ListAllDefinitions()` reports each tool's `Enabled` flag, and a server counts as on when at least one of its tools is enabled. A server disabled in config appears in neither `ToolNamesByServer` nor `FailedServers`, so `McpClientPool.DisabledServers` exists purely to keep it visible — without it, turning a server off would be a one-way door with no way to discover it again.
- **`/mcp-toggle` drives `IToolRegistry`'s user axis, not the permission system** — permissions are a *consent gate*: a denied tool is still advertised to the model and only fails at invoke time. The registry's `DisableToolByUser`/`EnableToolByUser` axis instead drops the tool out of `ListDefinitions()` entirely, so a disabled server's tools leave the model's advertised list on the next turn (and are refused immediately if called). That is why this feature toggles the registry rather than adding permission rules.
- **`Enabled: false` is honoured before the transport is created** — `McpClientPool.CreateAsync` skips a disabled server ahead of `CreateTransport`/`McpClient.CreateAsync`, so no subprocess or Docker container is spawned for it at all. A *failed* server is still treated as "on" for toggle purposes (it is enabled in config, it merely could not connect), so disabling one is a legitimate way to stop it retrying every launch.
- **The toggle is persisted with a targeted `JsonNode` edit, never a config round-trip** — `McpServerConfiguration.Persist` sets exactly the one `Enabled` key. Serializing `McpClientOptions` back would be actively harmful: `McpConfigResolver.Expand` mutates that object *in place* at startup, substituting `${RepoRoot}` with a machine-specific absolute path and `${GitHubToken}` with the user's live token — a round-trip would destroy portability and write a credential into the config file. The write target is `Path.Combine(env.ContentRootPath, "appsettings.json")`, which (because `ContentRootPath` is `AppContext.BaseDirectory`) is the **build-output copy**, matching `/model` and `Agent:UserId`; it survives restarts and ordinary rebuilds but is discarded by `dotnet clean`. Persistence is skipped under Test.
