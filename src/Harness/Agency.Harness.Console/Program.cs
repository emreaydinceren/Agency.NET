...
internal class Program
{
    public static async Task<int> Main(string[] args)
    {
        ...
        try
        {
            ...
            if (mcpOptions is { Servers.Length: > 0 })
            {
                ...
                try
                {
                    ...
                    foreach ((string name, string error) in pool.FailedServers)
                    {
                        System.Console.Error.WriteLine($"[Agency] MCP server '{name}' unavailable — skipped: {error}");
                    }
                ...
                {
                    // Never let MCP take down the app: a missing Docker/Node runtime or an unreachable
                    // server should degrade to "no MCP tools", not a failed boot.
                    System.Console.Error.WriteLine($"[Agency] MCP unavailable — continuing without MCP tools: {ex.Message}");
                }
            }
            ...
            // CI boot smoke check: when AGENCY_SMOKE_EXIT=1, startup has succeeded (DI built, schema
            // initialized, MCP connection attempted non-fatally) — exit 0 without entering the REPL so
            // CI can assert the app boots without a live LLM or interactive stdin.
            if (Environment.GetEnvironmentVariable("AGENCY_SMOKE_EXIT") == "1")
            {
                System.Console.WriteLine("[Agency] Smoke check: startup succeeded.");
                return 0;
            }
        ...
        {
            System.Console.Error.WriteLine($"[Agency] The console failed to start: {ex.Message}");
            return 1;
        }
        finally
        {
