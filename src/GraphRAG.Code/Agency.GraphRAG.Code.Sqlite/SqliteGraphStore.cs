...
public sealed class SqliteGraphStore : IGraphStore
{
        ...
        return this.RunOperationAsync(
            ...
            async activity =>
            {
                ...
                for (int index = 0; index < paths.Count; index++)
                {
                    ...
                    string parameterName = "path" + index.ToString(CultureInfo.InvariantCulture);
                    _ = sqlBuilder.Append('@').Append(parameterName);
                    parameters[parameterName] = paths[index];
                }

                _ = sqlBuilder.AppendLine(");");

                List<(Symbol Symbol, string Path)> rows = await this._sqliteRunner.QueryAsync(
                    sqlBuilder.ToString(),
    ...
    public Task<SourceFile?> GetFileByPathAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        return this.RunOperationAsync(
            ...
            async activity =>
            {
                ...
                List<SourceFile> results = await this._sqliteRunner.QueryAsync(
                    """
    ...
    private static SourceFile HydrateSourceFile(DbDataReader reader) =>
        ...
        {
            Id = ParseGuid(reader.GetString(0)),
            RepoId = ParseGuid(reader.GetString(1)),
            ProjectId = ParseGuid(reader.GetString(2)),
